...
import SpotPostsSection from './SpotPostsSection';
...
interface SpotDetailProps {
  ...
  // 기존 App.tsx 호환용. 지금 화면에서는 사용하지 않음.
  onReviewClick?: () => void;
}
...
export default function SpotDetail({
  ...
  if (!spot) {
    return (
      <section className="detail empty">
        <MapPinned size={32} />
        <h2>스팟을 선택해주세요</h2>
        <p>지도 마커나 왼쪽 목록을 누르면 촬영 포인트 정보가 나옵니다.</p>
      </section>
    );
  }
  ...
  const sunsetUrl = `https://www.google.com/search?q=${encodeURIComponent(
    `${spot.name} 일출 일몰 시간`
  )}`;
  ...
  return (
    <section className="detail">
      ...
      {spot.imageUrl && (
        <img className="heroImage" src={spot.imageUrl} alt={spot.name} />
      )}
      ...
      <div className="detailBody">
        ...
        <div className="scoreLine">
          <Star size={17} />
          {avg ? avg.toFixed(1) : '현장 기록'}
          <span>기록 {spot.reviews.length}개</span>
        </div>
        ...
        <div className="saveButtonRow">
          {savedButtonMeta.map((item) => (
            <button
              key={item.id}
              className={
                savedStatuses.includes(item.id) ? 'saveButton active' : 'saveButton'
              }
              onClick={() => onToggleSavedStatus(item.id)}
            >
              {item.icon}
              {item.label}
            </button>
        ...
        <div className="officialCheckBox">
          ...
          <p>
            이 앱은 촬영 스팟과 현장 후기를 모아보는 참고 지도야. 실제 비행
            가능 여부는 드론원스톱, DJI Fly Safe, 관제권·군·촬영금지시설 여부를
            따로 확인해야 해.
          </p>

          <button
            className="checklistToggle"
            onClick={() => setChecklistOpen((value) => !value)}
          >
            {checklistOpen ? '체크리스트 접기' : '비행 전 체크리스트 열기'}
          </button>
        ...
        {matchedZones.length > 0 && (
          <div className="zoneWarningBox">
            ...
            <p>
              샘플 레이어 기준으로 아래 구역과 겹칠 가능성이 있어. 실제 비행 전
              공식 확인 필수.
            </p>
        ...
        <div className="statGrid richStats">
          <div>
            <span>카테고리</span>
            <strong>{categoryLabel[spot.category]}</strong>
          </div>

          <div>
            <span>난이도</span>
            <strong>{difficultyLabel[spot.difficulty]}</strong>
          </div>

          <div>
            <span>주차</span>
            <strong>{parkingLabel[spot.parking]}</strong>
          </div>

          <div>
            <span>이륙 공간</span>
            <strong>{takeoffSpaceLabel[spot.takeoffSpace]}</strong>
          </div>

          <div>
            <span>사람 많음</span>
            <strong>{crowdLevelLabel[spot.crowdLevel]}</strong>
          </div>

          <div>
            <span>추천 시간</span>
            <strong>{spot.bestTime.join(', ')}</strong>
          </div>
        </div>

        <h3>현장 메모</h3>
        <p className="bodyText">{spot.description}</p>

        <h3>이륙 포인트</h3>
        <p className="bodyText">{spot.takeoffPoint}</p>

        {spot.viewPoints.length > 0 && (
          <>
            <h3>촬영 포인트</h3>
            <div className="tagWrap pointTags">
              {spot.viewPoints.map((item) => (
        ...
        <div className="coordinateBox">
          ...
          <button className="textButton" onClick={copyCoordinates}>
        ...
        <SpotPostsSection spot={spot} user={user} />
      </div>
    </section>
  );
}